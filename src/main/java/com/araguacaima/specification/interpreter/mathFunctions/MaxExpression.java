...
public class MaxExpression extends NonTerminalMathFunctionExpression {
    public MaxExpression(Expression l, Expression r) {
        super(l, r);
    }
    ...
    public Expression evaluate(Context c)
        ...
        if (getEvaluateAllTerms()) {
            ...
            return new MathFunctionExpressionImpl(Math.max(result1, result2));
        } else {
            return new MathFunctionExpressionImpl(
                    Math.max(Double.parseDouble(String.valueOf(getLeftNode().evaluate(c).getValue()))
